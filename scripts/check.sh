...
TC="$(cd "$1" && pwd)" || pa_die "no such directory: $1"
TESTS="${PA_ROOT}/test"
EXE="${PA_EXE}"
...
for t in ar gcov; do
    [ -x "${TC}/bin/${t}${EXE}" ] && pass "bin/${t} present" || fail "bin/${t} missing (rules_ada uses it)"
done
...
# is also the order gnatlink uses (-lgnarl -lgnat).
link_libs=("${ADALIB}/libgnarl.a" "${ADALIB}/libgnat.a" "${GCCLIB}/libgcc.a")
extra_link=()
case "${PA_OS}" in
    linux) extra_link=(-lpthread -ldl -lrt -lm) ;;   # needed on glibc < 2.34
esac
