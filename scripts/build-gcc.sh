...
if [ "${PA_OS}" = windows ]; then
    ...
    # The native system header dir (--with-native-system-header-dir above)
    # must exist when stage 1 runs fixincludes, and it should hold the real
    # system headers so that fixincludes and the installed compiler agree.
    # This is where msys2's GCC keeps them too (<prefix>/include).
    mkdir -p "${PA_PREFIX}/include"
    cp -a "${MINGW_PREFIX}/include/." "${PA_PREFIX}/include/"
fi
...
if [ "${PA_OS}" = windows ]; then
    # Configure with a *relative* source directory, as msys2's own GCC
    # packages do.  An absolute POSIX srcdir (/pa/work/src/...) reaches the
    # native gengtype.exe as C:/pa/work/src/... on its command line but as
    # /pa/work/src/... in the file list make writes for it; the prefix never
    # matches, gcc/except.cc and gcc/cp/except.cc both map to gt-except.h,
    # and stage 1 fails compiling except.o.  A relative path is identical in
    # both worlds.
    configure="$(realpath --relative-to="${bld}" "${src}")/configure"
fi

(
    cd "${bld}"
    pa_run "${configure}" "${args[@]}"
